...
// --- FAUCET ---
/**
 * Creates and executes the transaction block for claiming from the faucet.
 * @param {Function} signAndExecuteTransactionBlock - The function from the useWallet() hook.
 * @returns {Promise<any>} - The result of the transaction execution.
 */
export const createFaucetTx = async (signAndExecuteTransactionBlock) => {
  ...
  return signAndExecuteTransactionBlock({
    ...
    // Optional: You can add options here if needed, e.g., for sponsored transactions
    // options: {
    //   showEffects: true,
    // },
  });
